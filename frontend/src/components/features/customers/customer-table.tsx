import type { Customer } from "../../../types/customer";

interface CustomerTableProps {
  customers: Customer[];
  canManage: boolean;
  onEdit: (customer: Customer) => void;
  onDelete: (customer: Customer) => Promise<void>;
}

const statusClassNames = {
  active: "bg-emerald-50 text-emerald-700",
  inactive: "bg-slate-100 text-slate-600"
} as const;

export const CustomerTable = ({
  customers,
  canManage,
  onEdit,
  onDelete
}: CustomerTableProps) => {
  return (
    <section className="rounded-[1.75rem] bg-white p-4 shadow-[0_20px_60px_rgba(11,20,55,0.08)] sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-medium text-[#A3AED0]">Customer records</p>
        </div>
        <div className="rounded-full bg-[#F4F7FE] px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#4318FF]">
          Search and filter
        </div>
      </div>

      <div className="mt-6 space-y-4 md:hidden">
        {customers.length === 0 ? (
          <div className="rounded-2xl bg-[#F8FAFF] px-4 py-8 text-center text-sm font-medium text-[#707EAE]">
            No data
          </div>
        ) : (
          customers.map((customer) => (
            <article key={customer.id} className="rounded-[1.5rem] bg-[#F8FAFF] p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-base font-bold text-[#2B3674]">{customer.companyName}</h3>
                  <p className="mt-1 text-sm text-[#707EAE]">{customer.contactPerson}</p>
                </div>
                <span
                  className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${statusClassNames[customer.status]}`}
                >
                  {customer.status}
                </span>
              </div>

              <dl className="mt-4 space-y-3 text-sm text-[#707EAE]">
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-[#A3AED0]">Email</dt>
                  <dd className="mt-1 break-all">{customer.email}</dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-[#A3AED0]">Phone</dt>
                  <dd className="mt-1">{customer.phone}</dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-[#A3AED0]">Address</dt>
                  <dd className="mt-1">{customer.address}</dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-[#A3AED0]">Updated</dt>
                  <dd className="mt-1">{new Date(customer.updatedAt).toLocaleDateString()}</dd>
                </div>
              </dl>

              {canManage ? (
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <button
                    type="button"
                    className="inline-flex h-11 items-center justify-center rounded-full bg-white text-sm font-semibold text-[#4318FF]"
                    onClick={() => onEdit(customer)}
                  >
                    Edit
                  </button>
                  <button
                    type="button"
                    className="inline-flex h-11 items-center justify-center rounded-full bg-white text-sm font-semibold text-rose-600"
                    onClick={() => onDelete(customer)}
                  >
                    Delete
                  </button>
                </div>
              ) : null}
            </article>
          ))
        )}
      </div>

      <div className="mt-6 hidden overflow-x-auto md:block">
        <table className="min-w-full border-separate border-spacing-y-3 text-left">
          <thead>
            <tr className="text-xs uppercase tracking-[0.18em] text-[#A3AED0]">
              <th className="px-4">Company</th>
              <th className="px-4">Contact</th>
              <th className="px-4">Email</th>
              <th className="px-4">Phone</th>
              <th className="px-4">Status</th>
              <th className="px-4">Updated</th>
              {canManage ? <th className="px-4">Actions</th> : null}
            </tr>
          </thead>
          <tbody>
            {customers.length === 0 ? (
              <tr className="bg-[#F8FAFF]">
                <td
                  className="rounded-2xl px-4 py-8 text-center text-sm font-medium text-[#707EAE]"
                  colSpan={canManage ? 7 : 6}
                >
                  No data
                </td>
              </tr>
            ) : (
              customers.map((customer) => (
                <tr key={customer.id} className="bg-[#F8FAFF]">
                  <td className="rounded-l-2xl px-4 py-4 text-sm font-semibold text-[#2B3674]">
                    <div>{customer.companyName}</div>
                    <div className="mt-1 text-xs font-medium text-[#A3AED0]">{customer.address}</div>
                  </td>
                  <td className="px-4 py-4 text-sm text-[#707EAE]">{customer.contactPerson}</td>
                  <td className="px-4 py-4 text-sm text-[#707EAE]">{customer.email}</td>
                  <td className="px-4 py-4 text-sm text-[#707EAE]">{customer.phone}</td>
                  <td className="px-4 py-4">
                    <span
                      className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${statusClassNames[customer.status]}`}
                    >
                      {customer.status}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-sm text-[#707EAE]">
                    {new Date(customer.updatedAt).toLocaleDateString()}
                  </td>
                  {canManage ? (
                    <td className="rounded-r-2xl px-4 py-4">
                      <div className="flex gap-3 text-sm font-semibold">
                        <button
                          type="button"
                          className="text-[#4318FF]"
                          onClick={() => onEdit(customer)}
                        >
                          Edit
                        </button>
                        <button
                          type="button"
                          className="text-rose-600"
                          onClick={() => onDelete(customer)}
                        >
                          Delete
                        </button>
                      </div>
                    </td>
                  ) : null}
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>
    </section>
  );
};
